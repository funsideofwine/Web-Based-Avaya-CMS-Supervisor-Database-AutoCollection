...
if (isset($_POST["login"])){
    ...
    if(empty($email) || empty($password)){//1
      header("location: ../login1.php?header=error&msg=\"you need fillup all form fields\"");
      exit();
    }
    else
    {//2
    ...
    $resultCheck = mysqli_num_rows($result);
      if($resultCheck < 1) {//3
      header("location: ../login1.php?header=error&msg=\"account is not existing, Please Try Again!\"");
      exit();
    }//3
      else {//4
      ...
      $hashedpwd = password_verify($password, $row['user_pwd']);
        ...
        header("location: ../login1.php?header=error&msg=\"password mismatched!\"");
        exit();
      }//6
		...
		###############################################################################	
		header("location: ../search1.php?hostname=tmp&header=Login&msg=\"Loggin Success!\"");
        exit();
      }//7
    }//5
    }//4
  ...
  header("location: ../login1.php?header=Login&msg=\"Failed!\"");
  exit();
}//1
